<template>
	<div id="device_content">
		...
		<div class="modal_p2" v-show="modal2_state">
			...
			<div class="modal2_items">
				<div v-for="item in queryUser.colmInfo" @click="modal2_items_choose(item.colmId)" class="modal2_item">
					<img v-show="item.imgState" src="../assets/img/check_54.png">
					<img v-show="!(item.imgState)" src="../assets/img/uncheck_54.png">
					<span v-if="!item.subColmInfo[0]">{{item.colmValue}}</span>
					<span v-if="item.subColmInfo[0]&&item.subColmInfo[0].colmEnName=='Authorized'">{{item.colmValue+"代"+item.subColmInfo[0].colmValue+"处理"}}</span>
				</div>
			</div>
		...
		<div class="check_opinion">
			...
			<button class="opinion_cc" @click="$router.go(-1)">取 消</button>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		watch: {
			...
			'queryOrg.tree': function(curVal, oldval) {
				this.orgList = curVal
				console.log(JSON.stringify(this.orgList))
				for (var k in this.orgList) {
					if ((this.orgList)[k].type == 'org' && (this.orgList)[k].pid == null) {
						(this.orgList)[k].nodeStatus = true;
						this.initLevel = (this.orgList)[k].org.orgLevel
					} else if ((this.orgList)[k].type == 'org') {
						(this.orgList)[k].nodeStatus = true;
						if ((this.orgList)[k].org.orgLevel > this.finalLevel) {
							this.finalLevel = (this.orgList)[k].org.orgLevel
						}
					} else if ((this.orgList)[k].type == 'user') {
						(this.orgList)[k].checkStatus = false;
					}
				}
				this.$store.commit('setChoosePersonUnderOrg_orgList', this.orgList)
				this.$store.commit('setChoosePersonUnderOrg_initLevel', this.initLevel)
				this.$store.commit('setChoosePersonUnderOrg_finalLevel', this.finalLevel)
			},
		},
		created() {},
		beforeMount() {},
		...
		beforeRouteEnter(to, from, next) {
			next(vm => {
				if ((from.path == "/")||(from.path.search("/main")!=-1)) {
					vm.$indicator(1);
					vm.postCount = 0
					vm.usualOpinions = {
						auditingCommentsInfo: []
					...
					vm.login(() => {
						vm.$toast("已登录")
						vm.postPath(vm.postUsualOpinions())
					}, () => {
						vm.$toast("登录失败")
						vm.$indicator(0)
					})
		...
		methods: {
			login(success, err) {
				this.$login({
					j_username: this.getMainInfo.j_username,
					j_password: this.getMainInfo.j_password
				}, ()=>{
					this.$toast("登录成功")
					success()
				}, ()=>{
			...
			postPath(callback) {
				...
				var t = {
					procId: this.getMainInfo.procId,
					userId: this.getMainInfo.userId,
					system: this.getMainInfo.system,
					queryItem: qi
				};
			...
			postUsualOpinions(callback) {
				...
				var t = {
					userId: this.getMainInfo.userId,
					flowName: this.getMainInfo.types
				}
				...
				if (itm.colmEnName == "NextStep" && itm.colmId == 0) {
					...
					this.$confirmer("是否进行'" + path + "'", () => {
						...
						var comment = {
							colmEnName: "Comment",
							colmId: "Comment",
							colmValue: this.opinionContent
						};
						var step = {
							colmEnName: itm.colmEnName,
							colmId: itm.colmId,
							colmValue: itm.colmValue
						};
						var user = {
							colmEnName: "NextUser",
							colmId: "",
					...
					var qi = [{
						colmEnName: itm.colmEnName,
						colmValue: itm.colmId,
						reserve1: itm.colmValue
					}];
					var t = {
						procId: this.getMainInfo.procId,
						userId: this.getMainInfo.userId,
					...
					this.$post(this.$url.queryTpl, t).then((data) => {
						...
						if (data && data.colmInfo && data.colmInfo.length > 0) {
							...
							if (itm0.colmEnName == "NextUser") {
								...
								//会签-单选
								var treeStr = JSON.parse(itm0.colmValue)
								this.queryOrg.tree = treeStr.tree
								this.$router.push("/choosePersonUnderOrg")
							}else if(itm0.colmEnName == "TreeMultiNextUser"){
								console.log()
							}
						}
					}, (err) => {
			...
			modal2_y() {
				...
				this.$confirmer("是否提交给'" + person + "'进行'" + path + "'", () => {
					...
					var step = {
						colmEnName: pathItm.colmEnName,
						colmId: pathItm.colmId,
						colmValue: pathItm.colmValue
					};
					var user = {
						colmEnName: personItm.colmEnName,
						colmId: personItm.colmId,
						colmValue: personItm.colmValue
					};
					colmInfo.push(comment);
					colmInfo.push(step);
					colmInfo.push(user);
