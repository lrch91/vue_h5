...
export function callHandler(osType, method, data, func) {
    //osTYpe:0-android 1-ios
    if(osType==1){
        setupWebViewJavascriptBridge(function(bridge) {
            bridge.callHandler(method, data, func)
        });
    }else if(osType==0){
        if(window.jsInterFace){
            // window.jsInterFace.dataForCallback();
            var func = eval("window.jsInterFace."+method)
            func(data, func)
        }else{
...
export function registerHandler(osType, method, callback) {
    //osTYpe:0-android 1-ios
    if(osType==1){
        setupWebViewJavascriptBridge(function(bridge) {
